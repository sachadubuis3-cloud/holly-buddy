...
type RequestRow={id:string;host_organization_id:string;provider_id:string;requested_by:string|null;status:string;created_at:string};

export default function ProviderRequests({providerId}:Props){
 ...
 const load=async()=>{if(!supabase||!providerId){setLoading(false);return}setLoading(true);const {data,error}=await supabase.from('host_provider_connections').select('id,host_organization_id,provider_id,requested_by,status,created_at').eq('provider_id',providerId).eq('status','pending').order('created_at',{ascending:false});if(error)setMessage(`Impossible de charger les demandes : ${error.message}`);else setRows((data??[]) as RequestRow[]);setLoading(false)};
 useEffect(()=>{void load()},[providerId]);
 const decide=async(id:string,status:'connected'|'disconnected')=>{if(!supabase)return;setBusy(id);setMessage('');const {error}=await supabase.from('host_provider_connections').update({status,connected_at:status==='connected'?new Date().toISOString():null,disconnected_at:status==='disconnected'?new Date().toISOString():null}).eq('id',id).eq('provider_id',providerId);if(error)setMessage(`Impossible de traiter la demande : ${error.message}`);else await load();setBusy(null)};
